...
const handleDeleteProduct = (id) => {
  toast.custom((t) => (
    <div
      className="bg-white shadow-lg rounded-lg p-4 flex flex-col sm:flex-row sm:items-center gap-3 w-full max-w-sm"
    >
      <span className="text-black text-sm">Are you sure you want to delete this product?</span>
  ...
  const handleImageError = (e) => {
    ...
    e.target.className = "w-full h-auto max-h-96 object-contain p-4 bg-gray-100";
  };
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4">
            <div>
              ...
              <div className="text-xs sm:text-sm text-gray-600">
                <span className="block sm:inline-block sm:mr-4">Slug: {product.slug}</span>
                <span className="block sm:inline-block sm:mr-4">ID: {product._id}</span>
                {product.model && <span className="block sm:inline-block">Model: {product.model}</span>}
              </div>
            </div>
            <div className="flex gap-2 w-full sm:w-auto">
             
              <button
                onClick={() => navigate(`/admin/product/${product.slug}/analytics`)}
              ...
              >
               ...
               <button
                onClick={() => navigate(`/admin/product/update/${product.slug}`)}
  ...
  <path stroke-linecap="round" stroke-linejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" />
          ...
          <div className="border-b border-gray-200 mb-4">
            <nav className="flex space-x-4">
              ...
              <button
                ...
                className={`py-2 px-1 border-b-2 font-medium text-sm ${activeTab === "variants" ? "border-black text-black" : "border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300"}`}
              >
                Variants
              </button>
              <button
                onClick={() => setActiveTab("seo")}
                className={`py-2 px-1 border-b-2 font-medium text-sm ${activeTab === "seo" ? "border-black text-black" : "border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300"}`}
              >
                SEO
              </button>
            </nav>
          ...
          {activeTab === "details" && (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              ...
              <div className="space-y-6">
                {/* Basic Info */}
                <div>
                  <h2 className="text-lg font-semibold border-b pb-2 mb-3">Basic Information</h2>
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    ...
                    <div>
                      ...
                      <p className="font-medium">{product.discountPercentage || 0}%</p>
                    </div>
                    <div>
                      <p className="text-gray-600">Final Price</p>
                      <p className="font-medium">₹{product.finalPrice?.toLocaleString()}</p>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <p className="font-medium">{product.rating?.toFixed(1) || "0.0"} ⭐</p>
                    </div>
                    <div>
                    ...
                    {product.activityType && (
                      <div>
                        ...
                        <p className="font-medium capitalize">{product.activityType}</p>
                      </div>
                    )}
                    {product.sportType && (
                      <div>
                        <p className="text-gray-600">Sport Type</p>
                        <p className="font-medium capitalize">{product.sportType}</p>
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="mb-6 border-b">
                  ...
                  <h2 className="text-lg font-semibold pb-2 mb-3">Product Details</h2>
                ...
                {/* Flags */}
                <div>
                  <h2 className="text-lg font-semibold border-b pb-2 mb-3">Flags</h2>
                ...
                {/* Tags & Badges */}
                <div>
                  <h2 className="text-lg font-semibold border-b pb-2 mb-3">Tags & Badges</h2>
                ...
                {/* Stats */}
                <div>
                  <h2 className="text-lg font-semibold border-b pb-2 mb-3">Statistics</h2>
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
                    ...
                    <div className="border p-2 rounded">
                      ...
                      <p className="font-medium">{product.wishlistCount || 0}</p>
                    </div>
                    <div className="border p-2 rounded">
                      <p className="text-gray-600">In Cart</p>
                      <p className="font-medium">{product.addToBagCount || 0}</p>
                    </div>
                  </div>
                </div>
          ...
          {activeTab === "variants" && (
            <div className="bg-white rounded border">
              <h2 className="text-lg font-semibold border-b pb-2 mb-4">Product Variants</h2>
                        ...
                        <div className="md:w-1/3">
                          <div className="flex items-center gap-3 mb-3">
                            <div
                              ...
                              title={variant.color}
                            />
                            <h3 className="font-medium capitalize">{variant.color}</h3>
                        ...
                        <div className="md:w-2/3">
                          ...
                          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                            {variant.sizes?.map((size, sizeIndex) => (
                              <div 
                                key={sizeIndex} 
                                className={`border p-3 rounded text-sm ${size.stock === 0 ? 'bg-red-50' : 'bg-white'}`}
                              >
                                <p className="font-medium">Size: {size.size}</p>
                                <p className={size.stock === 0 ? 'text-red-500' : ''}>
                                  Stock: {size.stock}
                                </p>
                                {size.price && (
                                  <p className="text-gray-600">Price: ₹{size.price.toLocaleString()}</p>
                                )}
                              </div>
                            ))}
                          </div>
          ...
          {activeTab === "seo" && (
            <div className="bg-white rounded border p-4">
              <h2 className="text-lg font-semibold border-b pb-2 mb-4">SEO Information</h2>
              <div className="space-y-4">
                <div>
